...
export interface StxPostConditionForm {
  type: 'stx';
  address: string;
...
export interface FtPostConditionForm {
  type: 'ft';
  address: string;
  condition: FungibleComparator;
...
export interface NftPostConditionForm {
  type: 'nft';
  address: string;
...
export interface PostConditionsState {
  mode: 'allow' | 'deny';
  ...
  return conditions
    ...
    .map(pc => {
      ...
      if (pc.type === 'ft') {
        ...
        switch (pc.condition) {
          case 'eq':
            return principal.willSendEq(amount).ft(contractId as `${string}.${string}`, tokenName);
          case 'gt':
            return principal.willSendGt(amount).ft(contractId as `${string}.${string}`, tokenName);
          case 'gte':
            return principal.willSendGte(amount).ft(contractId as `${string}.${string}`, tokenName);
          case 'lt':
            return principal.willSendLt(amount).ft(contractId as `${string}.${string}`, tokenName);
          case 'lte':
            return principal.willSendLte(amount).ft(contractId as `${string}.${string}`, tokenName);
        }
      }
...
export const FUNGIBLE_COMPARATORS: { value: FungibleComparator; label: string }[] = [
...
export const NFT_COMPARATORS: { value: NonFungibleComparator; label: string }[] = [
